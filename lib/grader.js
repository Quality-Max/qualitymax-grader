 ...
 */
function gradeTest(code, filePath) {
  if (!code || !code.trim()) {
    return {
      score: 0,
      grade: 'F',
      checks: [{ id: 'empty', name: 'Non-empty file', maxPoints: 0, earned: 0, passed: false, issue: 'Empty file' }],
      issues: ['Empty file'],
    };
  }
  ...
  const hasTestImport = /(?:import|require).*(?:test|expect).*@playwright\/test/.test(code);
  {
...
module.exports = { gradeTest, scoreToGrade };
